...
QByteArray array_Single::deviceaction_s(int orderType)
{
    //上料:FC 0C 00 00 00 00 02 00 01 00 00 5A
    //卸料:FC 0C 00 00 00 00 02 00 02 00 00 5A
    QByteArray  ByteArray;
    ByteArray.append(char(0xFC));
    ByteArray.append(char(0x0C));
    QByteArray zero0;
    zero0.fill(0x00,4);
    ByteArray.append(zero0);
    ByteArray.append(char(0x02));
    ByteArray.append(char(0x00));
    ByteArray.append(char(orderType));    //1 上料   2卸料

    QByteArray zero;
    zero.fill(0x00,2);
    ByteArray.append(zero);
    ByteArray.append(0x05a);
    //qDebug()<<"deviceaction_s-->action: ByteArray:"<<ByteArray.toHex();
    return ByteArray;
}
