...
IconHelper::IconHelper(QObject *) : QObject(qApp)
{
    ...
    //int fontId = QFontDatabase::addApplicationFont(":/image/fontawesome-webfont.woff");
...
{
    iconFont.setPointSize(size*1.2);
    btn->setFont(iconFont);
    btn->setText(c);
}

QFont IconHelper::GetIcon(QChar c, quint32 size)
{
    iconFont.setPointSize(size*1.2);
    return  iconFont;
}
...
{
    QPixmap pix(pixWidth*1.2, pixHeight*1.2);
                          ...
                          const QString &normalTextColor, const QString &darkTextColor)
{
    ...
    if (type == "top") {
        strBorder = QString("border-width:%1px 0px 0px 0px;padding:%1px %2px %2px %2px;")
                .arg(borderWidth).arg(borderWidth);
    } else if (type == "right") {
...
void IconHelper::widgetsetStyle(QWidget *widget, QList<QToolButton *> btns, QList<int> pixChar, quint32 iconSize, quint32 iconWidth, quint32 iconHeight, QString type, int borderWidth, QString borderColor, QString normalBgColor, QString darkBgColor, QString normalTextColor, QString darkTextColor)
{
    int btnCount = btns.count();
    int charCount = pixChar.count();
    if (btnCount <= 0 || charCount <= 0 || btnCount != charCount) {
